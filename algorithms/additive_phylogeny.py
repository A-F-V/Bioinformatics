...
def additive_phylogeny(d_mat):
    global nextleaf
    nextleaf = d_mat.size
    def additive_phylogeny_aux(d_mat:DistanceMatrix):
        ...
        if d_mat.size==2:
            ...
            tree = additive_phylogeny_aux(d_trim
            )
            print(str(tree)+"\n"*3)
            ...
            for (a,weight,b) in path:
                ...
                if count==x:
                    ...
                    return tree
                if count>x:
                    m = tree.cut(a,b,weight-(count-x),nextleaf)
                    nextleaf +=1
                    tree.add_node(j,m,lj)
                    return tree
            raise Error 
    return additive_phylogeny_aux(d_mat)
